...
async function connectToWA() {
  ...
  // Define the socket connection and assign it to 'robin'
  ...
  robin.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      // Send messages to owner and a test number
...
if (
  ...
  await robin.readMessages([{ remoteJid: mek.key.remoteJid, id: mek.key.id }]);
    ...
    // Bot operating modes: private/inbox/groups
    if (!isOwner && config.MODE === "private") return;
    if (!isOwner && isGroup && config.MODE === "inbox") return;
    if (!isOwner && !isGroup && config.MODE === "groups") return;

    // Command handling via events from command.js
    ...
    // Additional command handling for different events
